    ...
    public void loadApplications(List<String> records) {
        ...
        for (String record : records) {
            ...
            applications.merge(id, incoming, (existing, candidate) -> {
                if (candidate.getCreditScore() != existing.getCreditScore()) {
                    return candidate.getCreditScore() > existing.getCreditScore()
                            ? candidate : existing;
                }
                if (Double.compare(candidate.getLoanAmount(), existing.getLoanAmount()) != 0) {
                    return candidate.getLoanAmount() < existing.getLoanAmount()
                            ? candidate : existing;
                }
                return candidate.getCustomerName().compareTo(existing.getCustomerName()) < 0
                        ? candidate : existing;
            });
        }
    }
