...
- `400 Bad Request` - Validation failure / malformed request / invalid params
- `404 Not Found` - Resource not found
- `405 Method Not Allowed` - Unsupported HTTP method for endpoint
- `409 Conflict` - Business conflict (for example insufficient stock)
- `500 Internal Server Error` - Unhandled backend exception

## Standard Error Response Format

All handled exceptions return this JSON structure:

```json
{
  "timestamp": "2026-07-10T16:10:30.123",
  "status": 400,
  "error": "Bad Request",
...
`details` is included when field/parameter-level validation errors are present.
...
- Bean validation (`@Valid`, query/path validation) -> `400 Bad Request`
- Type mismatch / malformed JSON / missing required query param -> `400 Bad Request`
...
- `title`, `author`, `publisher`: required, non-blank
- `price`: must be greater than `0`

### Product payload

- `name`, `description`, `category`, `brand`, `sku`: required, non-blank
- `price`: must be greater than `0`
- `discountPercentage`: `0` to `100`
- `taxPercentage`: `0` to `100`
- `stockQuantity`: `>= 0`
- `rating`: `0` to `5`
- `reviewCount`: `>= 0`

### Common query/path validation

- IDs must be greater than `0`
- Stock operation `quantity` must be greater than `0`
- Price filters use non-negative values
- Range APIs require `min <= max`
- `/api/products/discounted?minRating=` accepts `0` to `5`
